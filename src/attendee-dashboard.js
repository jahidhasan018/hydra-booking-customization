...
import AttendeeDashboard from './components/AttendeeDashboard.vue'
  ...
  if (container && !container.__vue_app__) {
    try {
      ...
      app.config.errorHandler = (err, vm, info) => {
        console.error('Vue Attendee Dashboard error:', err, info)
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', mountAttendeeApp)
} else {
...
setTimeout(mountAttendeeApp, 100)